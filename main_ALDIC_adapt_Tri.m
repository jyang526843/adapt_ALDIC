% ----------------------------------------------------------
% Example: Adaptive augmented Lagrangian Digital Image Correlation (adapt-ALDIC) 
%          method with Kuhn triangulation mesh
%
%
% Author: Jin Yang, Postdoc @UW-Madison, PhD @Caltech '19
% Email: aldicdvc@gmail.com  -or-  jyang526@wisc.edu
% Date: 2019.12, 2020.07
% ----------------------------------------------------------
...
for ImgSeqNum = 2:length(ImgNormalized)
    ...
    [U0,USubpb2,FSubpb2,udual,vdual,ConvItPerEle,ALSolveStep,coordinatesFEM,elementsFEM,dirichlet,neumann,mu,beta,...
    ...
    dirichlet=[]; alpha = beta; eleGeneration = ones(size(elementsFEM,1),1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    close all;
    Plotdisp_showTri(UIter,coordinatesFEMIter,elementsFEMIter(:,1:3));
    Plotstrain_showTri(FIter,coordinatesFEMIter,elementsFEMIter(:,1:3));
    ...
    solnIter = struct('UIter',UIter,'FIter',FIter,'udualIter',udualIter,'vdualIter',vdualIter,...
        ...
        'coordinatesFEMIter',coordinatesFEMIter,'elementsFEMIter',elementsFEMIter,'winsizeListIter',winsize*ones(size(coordinatesFEMIter,1),2),...
    ...
    while (LevelNo<6) && (winstepsize/(2^(LevelNo-1))>1) && (mod((winstepsize/(2^(LevelNo-1))),2)==0)
        ...
        [UIter,FIter,udualIter,vdualIter,USubpb2tempIter,ConvItPerEleIter,betamuIter,...
            coordinatesFEMIter,elementsFEMIter,eleGenerationIter,dirichletIter,neumannIter,winsizeListIter,...
            ...
            = funALMSIterTri(UIter,FIter,udualIter,vdualIter,USubpb2tempIter,...
            coordinatesFEMIter,elementsFEMIter,winstepsize,ClusterNo,...
            eleGenerationIter,dirichletIter,neumannIter,LevelNo,ImgNormalized{1},ImgNormalized{2},winsize,...
        ...
        solnIter = struct('UIter',UIter,'FIter',FIter,'udualIter',udualIter,'vdualIter',vdualIter,...
            'USubpb2tempIter',USubpb2tempIter,'ConvItPerEleIter',ConvItPerEleIter,'betamuIter',betamuIter,...
            'coordinatesFEMIter',coordinatesFEMIter,'elementsFEMIter',elementsFEMIter,'winsizeListIter',winsizeListIter,...
            'eleGenerationIter',eleGenerationIter,'dirichletIter',dirichletIter,'neumannIter',neumannIter, ...
   ...
   ResultSolnSeq{ImgSeqNum-1}=soln;  
...
%     
% [row,col] = find(coordinatesFEMIter(:,2)==183);
%  coordinatesFEMtemp = coordinatesFEMIter(row,:);
%  F11Itertemp = FIter(4*row-3);
%  %UyItertemp = UIter(2*row);
% hold on;  plot(coordinatesFEMtemp(:,1),F11Itertemp,'bo-');
% 
% 
% solnIter = ResultSolnSeq{1}{2};
% UIter = solnIter.UIter; FIter = solnIter.FIter; udualIter = solnIter.udualIter; vdualIter = solnIter.vdualIter;
% USubpb2tempIter = solnIter.USubpb2tempIter; ConvItPerEleIter = solnIter.ConvItPerEleIter; betamuIter = solnIter.betamuIter;
% coordinatesFEMIter = solnIter.coordinatesFEMIter; elementsFEMIter = solnIter.elementsFEMIter;
% eleGenerationIter = solnIter.eleGenerationIter; dirichletIter = solnIter.dirichletIter; neumannIter = solnIter.neumannIter;
...
%  F11Itertemp = FIter(4*row-3);
%  %UyItertemp = UIter(2*row);
% hold on;  plot(coordinatesFEMtemp(:,1),F11Itertemp,'r+');
% 
% 
% 
