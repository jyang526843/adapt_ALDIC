...
function [coordinatesFEM,elementsFEM,coordinates,elements,dirichlet,neumann,x,y,xCen,yCen,M,N] = MeshSetUpTri(x,y,xCen,yCen,winstepsize)

% ========= mesh for global method ===========
M = size(x,2);  N = size(x,1); % N is vertically in image; M is horizontally in image;
coordinatesFEM = zeros(M*N+(M-1)*(N-1),2);
...
xCen = xCen'; yCen = yCen';
for tempi = 1:((M-1)*(N-1))
    coordinatesFEM(M*N+tempi,:) = [xCen(tempi),yCen(tempi)];
end

elementsFEM = zeros(4*(M-1)*(N-1),3); % The factor 4 is because I cut one Q4 element into 4 Kuhn triangle elements
...
%    |/     \|
%    1-------2
% {2-5-1}; {4-5-2}; {3-5-4}; {1-5-3};

for j = 1:N-1
    for i = 1:M-1
        Node1=(j-1)*(M)+i;  Node2=(j-1)*(M)+i+1;    Node3=j*(M)+i; 
        Node4=j*(M)+i+1;    Node5=M*N+(j-1)*(M-1)+i;
        elementsFEM(4*((j-1)*(M-1)+i)-3,:) = [Node1, Node2, Node5];
        elementsFEM(4*((j-1)*(M-1)+i)-2,:) = [Node2, Node4, Node5];
        elementsFEM(4*((j-1)*(M-1)+i)-1,:) = [Node4, Node3, Node5];
        elementsFEM(4*((j-1)*(M-1)+i)-0,:) = [Node3, Node1, Node5];
    end
end
