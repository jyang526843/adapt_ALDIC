...
function [U, H, stepwithinwhile] = funICGN_Subpb1(x0,y0,...
                    Df,imgfNormalizedbc,imggNormalizedbc,winsize,...
...
x = [x0-winsize(1)/2 ; x0+winsize(1)/2 ; x0+winsize(1)/2 ; x0-winsize(1)/2]; % [coordinates(elements(j,:),1)];
y = [y0-winsize(2)/2 ; y0+winsize(2)/2 ; y0+winsize(2)/2 ; y0-winsize(2)/2];  % [coordinates(elements(j,:),2)];
...
tempf = imgfNormalizedbc([x(1):1:x(3)],[y(1):1:y(3)]);
...
while(  stepwithinwhile <= 100 && normOfWNew > tol  )
           
    stepwithinwhile = stepwithinwhile+1;

    % Find region for g
    tempCoordxMat = XX - x0*ones(winsize(1)+1,winsize(2)+1);
    tempCoordyMat = YY - y0*ones(winsize(1)+1,winsize(2)+1);
    u22 = (1+P(1))*tempCoordxMat + P(3)*tempCoordyMat + (x0+P(5))*ones(winsize(1)+1,winsize(2)+1);
    v22 = P(2)*tempCoordxMat + (1+P(4))*tempCoordyMat + (y0+P(6))*ones(winsize(1)+1,winsize(2)+1);
    % [tempCoordy, tempCoordx] = meshgrid(y(1):y(3),x(1):x(3));
    % 
    ...
    if ~isempty([row1; row2; row3; row4])
        ...
        tempg = ba_interp2(imggNormalizedbc, v22, u22, 'cubic');
        % ====== Old version codes ======
...
%         end
        ...
        b2 = zeros(6,1); 
        tempfMinustempg = (tempf-meanf*ones(winsize(1)+1,winsize(2)+1))/bottomf - (tempg-meang*ones(winsize(1)+1,winsize(2)+1))/bottomg;
