...
function [x,y,u,v,cc,gridx,gridy] = funIntegerSearch(f,g,tempSizeOfSearchRegion,gridx,gridy,winsize,winstepsize,tempNoOfInitPt,varargin)
...
switch tempNoOfInitPt % 0- whole field; 1- several seeds points;
    case 0 % 0- whole field;
        ...
        while gridx(1)-tempSizeOfSearchRegion(1)-0.0*winsize < 1
            gridx(1) = gridx(1) +1;
        end
        while gridy(1)-tempSizeOfSearchRegion(1)-0.0*winsize < 1
            gridy(1) = gridy(1) +1;
        end
        while gridx(end)+1.0*winsize+tempSizeOfSearchRegion(2) > size(f,1)
            gridx(end) = gridx(end)-1;
        end
        while gridy(end)+1.0*winsize+tempSizeOfSearchRegion(2) > size(f,2) 
...
XList = [gridx(1) : winstepsize(1) : gridx(2) ];
YList = [gridy(1) : winstepsize(2) : gridy(2) ];
