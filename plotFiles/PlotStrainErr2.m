...
% Sample-14
   ErrStrain(i) = sqrt( (FSubpb2(4*i-3) - Sample14Exact(coordinatesFEM(i,1),7)*1e-6)^2 + (FSubpb2(4*i))^2 + (FSubpb2(4*i-2))^2 + (FSubpb2(4*i-1))^2 );
...
%     ExactStrainxx(i) = 0;
     ExactStrainxx(i) =  Sample14Exact(x(i,1),7)*1e-6;
  end
